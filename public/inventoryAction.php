...
if (!empty($_POST) && $_POST['action'] == 'list') {
    ...
    if (empty($_SESSION['searchCategory'])) {$_SESSION['searchCategory'] = "%";};
    ...
    $searchField .= "AND category LIKE '" . $_SESSION['searchCategory'] . "' ";
    ...
    $sqlQuery = "SELECT id, name, code, category, image, price, description FROM product WHERE " . $searchField;
        ...
        <tbody>
            <?php while ($product = mysqli_fetch_assoc($resultSet)) { ?>
                <tr id="<?php echo $product['id']; ?>">
                    ...
                    <td data-input="searchCategory"><?php echo $product['category']; ?></td>
